...
function PlatoonExceptionComponent(props: {
  ...
  useEffect(() => {
    setIsChecked(
      !props.missionDay.excludedSoldiers.some((s) => {
        return s.platoon === props.platoonNum;
      })
    );
  }, []);
  ...
  const handleSelectedPlatoon = (
    _: React.ChangeEvent<HTMLInputElement>,
    checked: boolean
  ) => {
    const shouldBeExcluded = !checked;
    props.missionDay.setPlatoonExcludedStatus(
            ...
            {companyContext.company.soldiers
              ...
              .map((soldier: Soldier) => (
                <div key={soldier.name}>
                  <SoldierExceptionComponent
                    handleSelectedSoldier={handleSelectedSoldier}
                    missionDay={props.missionDay}
                    soldier={soldier}
                  />
                </div>
              ))}
